...
class InternshipModel {
    ...
    public static function getStudentInternship($conn, $niu_estudiante){
                ...
                try{
                    ...
                    if(!empty($res)){
                        $internship = new Internship(
                            ...
                            $res['fecha_fin'], $res['id_tutor_externo'], $res['id_empresa'], $res['finalizada'], $res['id_curso_grado'],
                            );
                    }
     ...
     public static function getInternshipById($conn, $id_estancia){
        $internship = null;

            if(isset($conn)){
                try{
                    include_once '../entities/Internship.inc.php';
                    $sql = "SELECT * FROM estancias e WHERE e.id_estancia = :id_estancia";
                    $stmt = $conn -> prepare($sql);
                    ...
                    if(count($res)){
                        foreach($res as $intern){
                            $internships[] = new Internship(
                                ...
                                $intern['fecha_fin'], $intern['id_tutor_externo'], $intern['id_empresa'], $intern['finalizada'], $intern['id_curso_grado'],
                                );
                        } 
                     }
